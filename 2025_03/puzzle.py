...
def puzzle1(lines: list[str]):
    total = 0
    for l in lines:
        nums: list[int] = [int(c) for c in l.strip()]
        first_max_i, first_max_n = i_max(nums[:-1])
        _, second_max_n = i_max(nums[first_max_i + 1:])
        dp.print(f'{nums=} {first_max_i=} {first_max_n=} {second_max_n=}')
        dp.print(first_max_n * 10 + second_max_n)
        total += int(first_max_n * 10 + second_max_n)
    return total
...
def puzzle2(lines: list[str]):
    ...
    for l in lines:
        ...
        for col in range(COLS, 0, -1):
            max_i, max_n = i_max(nums[:len(nums) - (col - 1)])
            value = value * 10 + max_n
            nums = nums[max_i + 1:]
            dp.print(f'{nums=} {value=} {max_i=} {max_n=}')
        total += value
    return total
