...
def is_invalid2(n):
    s = str(n)
    for sublength in range(1, len(s) // 2 + 1):
        if len(s) % sublength != 0:
            continue
        substrs = [s[i * sublength:(i + 1) * sublength] for i in range(len(s) // sublength)]
        if all([substrs[0] == s for s in substrs[1:]]):
            return True
    return False
...
def is_invalid(n):
    s = str(n)
    return len(s) % 2 == 0 and s[0:len(s) // 2] == s[len(s) // 2:]
...
def puzzle1(lines: list[str]):
    ...
    total = 0
    for r in ranges:
        start, stop = (int(v) for v in r.split('-'))
